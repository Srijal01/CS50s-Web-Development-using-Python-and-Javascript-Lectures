...
In case you're worried about it, know that you'll never have to memorize these elements. It's very easy to simply search something like "image in HTML" to find the img tag. One resource that's especially helpful for learning about these elements is [W3 Schools](https://www.w3schools.com/).
...
Just as with other HTML elements, there's no need to memorize these, and [W3 Schools](https://www.w3schools.com/) is a great resource for learning about them!
...
Here, we use the descendant selector to only apply styling to list items found within an unordered list:
...
Not only can we use CSS to change what an element looks like permanently, but also what it looks like under certain conditions. For example, what if we wanted a button to change color when we hover over it? We can acheive this using a **CSS pseudoclass**, which provides additional styling during special circumstances. We write this by adding a colon after our selector, and then adding the circumstance after that colon.
...
Next, we can look at some of bootstrap's features by navigating to the [documentation](https://getbootstrap.com/docs/4.5/getting-started/introduction/) portion of their website. On this page, you'll find many examples of classes you can add to elements that allow them to be styled with bootstrap.
